...
import com.collect.user_luo.mycollect.sqlite.ormlite.OrmliteBaseHelper;
...
public class MyBaseDao {
    ...
    private Dao<MyBaseBean, Integer> dao;
    ...
    public MyBaseDao(Context context) {
        try {
            ...
            this.dao = ormliteBaseHelper.getDao(MyBaseBean.class);
    ...
    public MyBaseDao getInstance(Context context) {
        if (baseDao == null) {
            baseDao = new MyBaseDao(context);
        }
        return baseDao;
    }

    // 添加数据
    public int insert(MyBaseBean data) {
        try {
    ...
    // 添加数据集
    public int inserts(List<MyBaseBean> datas) {
        try {
            //noinspection unchecked
    ...
    // 删除数据
    public int delete(MyBaseBean data) {
        try {
    ...
    // 删除数据
    public void delete2(MyBaseBean data) {
        try {
...
//            FROM samplerules f   GROUP BY DATE_FORMAT(f.created_at,'%Y-%m-%d')
    ...
    // 修改数据
    public int update(MyBaseBean data) {
        int rows = -1;
        try {
     ...
     * @return
     */
    public int updates(List<MyBaseBean> datas) {
        int rows = -1;
        try {
            for (MyBaseBean bean : datas) {
    ...
    // 修改或者插入数据
    public boolean updateOrInsert(MyBaseBean data) {
        try {
    ...
    //    // 通过列名对应值查找
    public List<MyBaseBean> queryByColumnName(String columnName, Object value) {
        try {
    ...
    // 通过ID查询一条数据
    public MyBaseBean queryById(int id) {
        MyBaseBean bean = null;
        try {
    ...
    // 通过条件查询文章集合（通过用户ID查找）
    public List<MyBaseBean> queryByUserName(String COLUMNNAME_NAME, String userName) {
        try {
     ...
     * @return
     */
    public List<MyBaseBean> queryByDay(String day, boolean isAsce) {
        try {
    ...
    // 查找所有bean
    public List<MyBaseBean> queryAll() {
        try {
...
//        }
    ...
    // 查找所有bean
    public List<MyBaseBean> queryAllTest() {
        try {
